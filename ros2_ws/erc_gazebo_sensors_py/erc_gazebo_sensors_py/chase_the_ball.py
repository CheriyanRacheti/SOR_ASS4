    ...
    def display_image(self):
        ...
        while rclpy.ok():
            ...
            if self.latest_frame is not None:
                ...
                # 1. Process the current image using the original 320x240 frame
                mask, contour, crosshair = self.process_image(self.latest_frame)

                # 2. ENLARGE the main frame to 800x600 so the small images have room to fit
                display_frame = cv2.resize(self.latest_frame, (800, 600))

                # 3. Add processed images on top of the newly enlarged image
                result = self.add_small_pictures(display_frame, [mask, contour, crosshair])
